...
st.markdown("""
    ...
    .report-card-expenses { background: rgba(239, 68, 68, 0.1); border: 1px solid #ef4444; border-radius: 12px; padding: 20px; text-align: center; }
...
def extract_amount_from_text(text):
    # البحث عن أي أرقام داخل النص
    numbers = re.findall(r'\d[\d,]*', text)
    if numbers:
        # تنظيف الرقم من الفواصل وتحويله لقيمة عددية
...
# ==================== 🌟 المرحلة الثانية: إضافة الالتزامات والمصاريف ====================
    ...
    st.markdown("### 📝 اكتب الالتزامات (يدعم الكتابة الطبيعية باللغة العربية)")
    st.caption("مثال: يمكنك كتابة '50000 مال نت' أو 'ضرائب 30000' أو 'مصاريف البيت 100000' وسيقوم النظام بحسابها تلقائياً.")
    ...
    if st.button("➕ إدراج الالتزام فوراً في الحسبة الذكية"):
        if user_input_text:
            ...
            if extracted_val and extracted_val > 0:
                # تنظيف النص ليكون هو اسم المصروف واستبعاد الأرقام منه ليكون مرتباً
                cleaned_name = re.sub(r'\d[\d,]*', '', user_input_text).replace('مال', '').replace('من', '').strip()
                if not cleaned_name:
                    cleaned_name = "مصروف مضاف"
                ...
                st.error("⚠️ يرجى كتابة المبلغ والأرقام بشكل واضح داخل النص ليتمكن النظام من قراءتها.")

    # عرض الالتزامات الحالية إن وجدت
    if st.session_state.expenses:
        st.markdown("<br>📋 **الالتزامات الثابتة المسجلة حالياً:**", unsafe_allow_html=True)
...
# ==================== 🌟 المرحلة الثالثة: التقرير المالي النهائي الحاسم ====================
    ...
    # عرض المبالغ المقفلة والمحمية بكروت كلاس (3 كروت بالصف الأول)
    c1, c2, c3 = st.columns(3)
    with c1:
        st.markdown(f"""
            <div class="report-card-save">
                <span style="color:#10b981; font-weight:bold; font-size:13px;">🔒 الادخار المقفل (15%)</span>
                <h3 style="color:#fff; margin:10px 0 0 0;">{savings:,} {symbol}</h3>
            </div>
        """, unsafe_allow_html=True)
    with c2:
        st.markdown(f"""
            <div class="report-card-invest">
                <span style="color:#3b82f6; font-weight:bold; font-size:13px;">📈 الاستثمار المقفل (15%)</span>
                <h3 style="color:#fff; margin:10px 0 0 0;">{investment:,} {symbol}</h3>
            </div>
        """, unsafe_allow_html=True)
    with c3:
        st.markdown(f"""
            <div class="report-card-expenses">
                <span style="color:#ef4444; font-weight:bold; font-size:13px;">💸 الالتزامات والضرائب</span>
    ...
    if st.session_state.expenses:
        with st.expander("📋 عرض تفاصيل قائمة الالتزامات والضرائب التي قمت بإضافتها"):
